...
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get('DATABASE_URL', 'sqlite:///entrix.db')
...
def bills():
    ...
    return render_template('bills.html', user=user,
                           accounts=QB_ACCOUNTS, mira_tins=MIRA_TINS)
...
def api_post_bill():
    ...
    return jsonify({
        ...
        'qb_error': qb_error,
        'status': bill.status,
        'message': f'Bill saved to Entrix' + (f' and posted to QuickBooks (ID: {qb_bill_id})' if qb_bill_id else ' (connect QuickBooks to post)')
    })
...
def api_vendors_search():
    ...
    results = [{'id': v.id, 'name': v.name, 'tin': v.tin, 'tin_exempt': v.tin_exempt}
               for v in vendors if q in v.name.lower()]
