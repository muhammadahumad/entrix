...
PLANS = {
    'free':     {'name': 'Free',     'price': 0,  'scans': 10,  'qb_posting': False, 'bank': False},
    'starter':  {'name': 'Starter',  'price': 5,  'scans': 100, 'qb_posting': True,  'bank': False},
    'business': {'name': 'Business', 'price': 12, 'scans': 999999, 'qb_posting': True, 'bank': True},
}
...
class User(db.Model):
    ...
    scans_reset_date = db.Column(db.DateTime)
    ...
    def scans_remaining(self):
        self.reset_scans_if_needed()
        plan = self.get_plan()
        if plan['scans'] >= 999999:
            return 999999
        return max(0, plan['scans'] - (self.scans_this_month or 0))

    def reset_scans_if_needed(self):
        now = datetime.utcnow()
        if not self.scans_reset_date or now >= self.scans_reset_date:
            self.scans_this_month = 0
            self.scans_reset_date = datetime(now.year, now.month, 1) + timedelta(days=32)
            self.scans_reset_date = self.scans_reset_date.replace(day=1)
            db.session.commit()

    def can_scan(self):
        return self.scans_remaining() > 0

    def can_post_to_qb(self):
        return self.get_plan()['qb_posting']

    def increment_scans(self):
        self.reset_scans_if_needed()
        self.scans_this_month = (self.scans_this_month or 0) + 1
        db.session.commit()
...
def dashboard():
    ...
    return render_template('dashboard.html', user=user, recent_bills=recent_bills,
                           ...
                           plan=plan, scans_remaining=scans_remaining)
...
def billing():
    ...
    plan = user.get_plan()
    scans_remaining = user.scans_remaining()
    scans_used = user.scans_this_month or 0
    return render_template('billing.html', user=user, plan=plan,
                           scans_remaining=scans_remaining, scans_used=scans_used,
                           plans=PLANS)
...
def pricing():
    user = current_user()
    return render_template('pricing.html', plans=PLANS, user=user)
...
def api_extract_bill():
    ...
    if not user.can_scan():
        ...
        return jsonify({
            'ok': False,
            'error': f'You have used all {plan["scans"]} scans this month. Upgrade your plan to scan more bills.',
            'upgrade': True
        })
...
def api_extract_bank():
    ...
    if not user.get_plan()['bank']:
        return jsonify({
            'ok': False,
            'error': 'Bank statement upload is available on the Business plan. Upgrade to unlock.',
            'upgrade': True
        })
...
def api_post_bill():
    ...
    qb_bill_id = None
    qb_error = None
    if user.can_post_to_qb() and user.qb_access_token and user.qb_realm_id:
        ...
        if qb_bill_id:
            ...
            bill.status = 'saved'
    elif not user.can_post_to_qb():
        bill.status = 'saved'
        qb_error = 'QB posting requires Starter or Business plan'
    else:
        bill.status = 'saved'
